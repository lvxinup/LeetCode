/*思路 首先 比如说目标值是4，那么如果我们一直向前走，直到累加步数 sum 正好大于等于target时，有： 0 + 1 = 1 1 + 2 = 3 3 + 3 = 6 我们发现 sum 超过了目标值 4，超过的距离为2，是偶数，那么实际上我们只要将加上距离为 1 的时候，不加 1，而是 -1，那么此时累加和就损失了 2，那么正好能到目标值 4，如下： 0 - 1 = -1 -1 + 2 = 1 1 + 3 = 4 所以这种情况下，最小的步数就是累加步数到 sum 所用的步数 n。
